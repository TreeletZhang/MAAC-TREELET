...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        world.dim_c = 5  # communication channel dimensionality=5
        num_listeners = 4
        num_speakers = 4
        num_landmarks = 6
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            agent.state.p_pos = np.random.uniform(-1,+1, world.dim_p)  # world.dim_p=2
            agent.state.p_vel = np.zeros(world.dim_p)  # [0,0]
            agent.state.c = np.zeros(world.dim_c)  # [0,0,0,0,0]
        for i, landmark in enumerate(world.landmarks):
            landmark.state.p_pos = np.random.uniform(-1,+1, world.dim_p)
            landmark.state.p_vel = np.zeros(world.dim_p)  # [0,0]
        self.reset_cached_rewards()
    ...
    def observation(self, agent, world):
        if agent.listener:
            ...
            obs += [agent.speak_ind == np.arange(len(world.speakers))]  # 4 维 [True,False,False,False]
            # give listener communication from its speaker
            obs += [world.speakers[agent.speak_ind].state.c]  # 对应的speaker生成的communication  5维
            # give listener its own position/velocity,
            obs += [agent.state.p_pos, agent.state.p_vel]  # 4维
            ...
            return np.concatenate(obs)  # [True,False,False,False]变为[1,0,0,0]
        else:  # speaker
            obs = []
            # give speaker index of their listener
            obs += [agent.listen_ind == np.arange(len(world.listeners))]  # 4维
            # speaker gets position of listener and goal
            obs += [agent.goal_a.state.p_pos, agent.goal_b.state.p_pos]  # 对应的listener的位置信息，listerner对应的同颜色目标的位置信息  4维
