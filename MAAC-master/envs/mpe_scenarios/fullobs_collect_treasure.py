...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        world.dim_c = 2  #communication channel dimensionality=2
        num_agents = 8  # 一共8个智能体
        num_collectors = 6  # 其中6个是收集者，2个是bank
        num_deposits = num_agents - num_collectors  # 存储宝藏的bank数量=2
        world.treasure_types = list(range(num_deposits))  # 宝藏的类型数量=bank的数量 [0,1]
        world.treasure_colors = np.array(
            sns.color_palette(n_colors=num_deposits))
        num_treasures = num_collectors  #宝藏的数量=收集智能体的数量，如果被吃掉随机生成
        # add agents
        ...
        for i, agent in enumerate(world.agents):
            ...
            if agent.collector:
                ...
                agent.d_i = i - num_collectors  # 宝藏存储者bank的index
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.respawn_prob = 1.0  # 宝藏的重生概率
        ...
        for l in world.landmarks:
            if l.alive:
                for a in self.collectors(world):
                    if a.holding is None and self.is_collision(l, a, world):  # 如果智能体当前没有hold任何宝藏，并且和lm碰撞了
                        l.alive = False   # 
                        a.holding = l.type  # 把智能体a的holding记录为lm的类型，0或者1
    ...
    def get_agent_encoding(self, agent, world):
        encoding = []
        n_treasure_types = len(world.treasure_types) # 2
        if agent.collector:
            encoding.append(np.zeros(n_treasure_types))  # [0,0]
            encoding.append((np.arange(n_treasure_types) == agent.holding))  # 表示hold的宝藏是哪一钟，假设智能体hold的宝藏类型是0，append[True, False]
        else:
            encoding.append((np.arange(n_treasure_types) == agent.d_i))  # 表示bank是哪一种，假设bank的类型是1，append[False,True]
            encoding.append(np.zeros(n_treasure_types))  # [0,0]
        return np.concatenate(encoding)

    def observation(self, agent, world):
        n_visible = 7  # number of other agents and treasures visible to each agent 智能体可以看到的宝藏/其他智能体数量
        # get positions of all entities in this agent's reference frame
        other_agents = [a.i for a in world.agents if a is not agent]  # len=7
        closest_agents = sorted(
            zip(world.cached_dist_mag[other_agents, agent.i],
                other_agents))[:n_visible]
        treasures = [t.i for t in self.treasures(world)]  # len=6
        ...
        if agent.collector:
            ...
            obs.append((np.arange(n_treasure_types) == agent.holding))  # 智能体hold的宝藏，如果是宝藏1,就append[False,True]
        for _, i in closest_agents:  # 智能体每一个其他智能体的距离，其他智能体的速度信息、
            a = world.entities[i]
            obs.append(world.cached_dist_vect[i, agent.i]) # 智能体每一个其他智能体的距离,2维
            obs.append(a.state.p_vel)
            obs.append(self.get_agent_encoding(a, world))  # 4维。其他collector当前hold的宝藏类型/其他bank收藏的宝藏类型
        for _, i in closest_treasures:
            t = world.entities[i]
            obs.append(world.cached_dist_vect[i, agent.i])  # 智能体和每一个宝藏的距离信息，2维
            obs.append((np.arange(n_treasure_types) == t.type)) # 宝藏类型
